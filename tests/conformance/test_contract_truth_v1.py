"""Contract Truth v1 gate against the live GTEx MCP registry."""
...
async def test_documentation_matches_live_mcp_contract(
    ...
    monkeypatch.delenv("GTEX_LINK_MCP_PROFILE", raising=False)
    monkeypatch.chdir(tmp_path)

    from gtex_link.mcp import service_adapters
    from gtex_link.mcp.facade import create_gtex_mcp

    def forbid_service_construction(*args: object, **kwargs: object) -> None:
        raise AssertionError("contract discovery must not construct the GTEx service")

    monkeypatch.setattr(service_adapters, "GTExClient", forbid_service_construction)

    tools = await create_gtex_mcp(profile="full").list_tools()
