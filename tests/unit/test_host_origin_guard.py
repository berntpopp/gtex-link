"""Host/Origin boundary contracts for GTEx's HTTP applications."""
...
from gtex_link import server_manager
from gtex_link.config import ServerSettings, settings
...
def client(monkeypatch: pytest.MonkeyPatch) -> TestClient:
    monkeypatch.setattr(
        settings,
        "allowed_hosts",
        ["localhost", "127.0.0.1", "::1", "gtex-link.genefoundry.org"],
    )
    monkeypatch.setattr(settings, "allowed_origins", ["https://genefoundry.org"])
    application = server_manager.create_unified_app()
...
@pytest.mark.parametrize("host", ["gtex-link.genefoundry.org", "gtex-link.genefoundry.org:443"])
...
def test_absent_and_configured_origins_are_allowed(client: TestClient) -> None:
    no_origin = client.get("/mcp", headers={"Host": "gtex-link.genefoundry.org"})
    configured = client.get(
        "/mcp",
        headers={
            "Host": "gtex-link.genefoundry.org",
...
def test_json_environment_allowlists_are_parsed(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv(
        "GTEX_LINK_ALLOWED_HOSTS",
        '["localhost","gtex-link.genefoundry.org"]',
    )
    monkeypatch.setenv("GTEX_LINK_ALLOWED_ORIGINS", '["https://genefoundry.org"]')

    configured = ServerSettings(_env_file=None)

    assert configured.allowed_hosts == ["localhost", "gtex-link.genefoundry.org"]
...
def test_image_default_uses_guarded_unified_server_and_explicit_health_host() -> None:
    ...
    assert 'CMD ["gtex-link", "serve", "--transport", "unified"' in dockerfile
