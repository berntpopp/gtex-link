...
Documented exception: the ``search`` / ``fetch`` deep-research pair is the OpenAI
deep-research / Apps SDK contract. ``fetch`` is not a canonical verb, so the pair
is explicitly allowlisted here rather than renamed.
"""
...
_NAMESPACE = "gtex"
# Deep-research contract pair (ChatGPT / OpenAI Apps SDK); verbatim names are
# required by that integration, so they are exempt from the canonical-verb rule.
_DEEP_RESEARCH_ALLOWLIST = frozenset({"search", "fetch"})
...
async def test_tool_names_conform_to_standard_v1(facade: Any) -> None:
    ...
    for name in names:
        ...
        if name not in _DEEP_RESEARCH_ALLOWLIST:
