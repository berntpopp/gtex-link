...
`gtex-link` is a thin FastMCP backend in the GeneFoundry fleet. It is unauthenticated
...
```bash
gh api -X PATCH repos/berntpopp/gtex-link \
...
```bash
gh api repos/berntpopp/gtex-link --jq '.security_and_analysis'
```
