    ...
    if is_tool_in_profile("search", profile):
        ...
        async def search(query: str) -> dict[str, Any]:
            async def call() -> dict[str, Any]:
                ...
                # Title carries only the curated gene symbol + GENCODE ID -- the
                # upstream free-text GENCODE `description` is deliberately NOT
                # embedded in this flat ChatGPT document (a flat `title` cannot
                # carry the v1.1 typed `untrusted_text` envelope). The fenced
                # descriptor is available via `get_gene_information`.
                items = [
                    {
    ...
    if is_tool_in_profile("fetch", profile):
        ...
        async def fetch(id: str) -> dict[str, Any]:
            ...
            try:
                ...
                # The upstream free-text GENCODE `description` is deliberately
                # omitted from this flat ChatGPT document -- a flat `title`/
                # `text` cannot carry the v1.1 typed `untrusted_text` envelope,
                # and ChatGPT results do not need the free-text descriptor. The
                # fenced descriptor is available via `get_gene_information`. Only
                # curated identifiers, enums, and numeric coordinates are
                # embedded here.
                lines = [
