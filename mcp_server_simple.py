        ...
        if name == "search_gtex_genes":
            ...
            return [TextContent(
                ...
                text=f"Found {len(genes)} genes:\n\n" + 
                     "\n".join([f"• {g['gene_symbol']} ({g['gencode_id']}): {g['description']}" for g in genes])
            )]
            ...
            return [TextContent(
                ...
                text=f"Found {len(expressions)} expression records:\n\n" +
                     "\n".join([f"• {e['gene_symbol']} in {e['tissue']}: {e['median_tpm']:.2f} TPM ({e['num_samples']} samples)" 
                               for e in expressions[:10]])  # Limit to first 10
            )]
            ...
            return [TextContent(
                ...
                text=f"Top {len(genes)} expressed genes in {tissue}:\n\n" +
                     "\n".join([f"{i+1}. {g['gene_symbol']}: {g['median_tpm']:.2f} TPM" 
