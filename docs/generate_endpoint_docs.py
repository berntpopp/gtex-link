...
def generate_endpoint_markdown(path: str, method: str, endpoint_data: Dict[str, Any]) -> str:
    ...
    if any(p.get('name') == 'gencodeId' for p in parameters):
        markdown += "- Use versioned GENCODE IDs (e.g., 'ENSG00000065613.9') for consistent results\n"
    ...
    markdown += f"- Always handle HTTP error responses (400, 422, etc.) appropriately\n"
    markdown += f"- Check the response schema reference for detailed field information\n"
    
    return markdown


def main():
    """Main function to generate endpoint documentation."""
